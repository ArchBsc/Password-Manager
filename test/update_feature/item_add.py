...
def new_item():
    ...
    while count <= int(item_number):
        ...
        if item_number == 1:
            ...
            b = str(input())
            print(
                "[#14BDFF]Do you want to generate the password[/#14BDFF] (type yes to generate);")
            pass_gen = input(": ")
            if pass_gen == "yes":
                h = str(password_gen())
            else:
...
def item_search(pattern):
    ...
    if pattern == "all":
        items = c.fetchall()
        number = 1
        table_date = [["[#0E7C83]id[/#0E7C83]", "[#14BDFF]Username[/#14BDFF]",
                       "[#14BDFF]Email[/#14BDFF]", "[#14BDFF]Password[/#14BDFF]", "[#14BDFF]Url[/#14BDFF]"]]
        for item in items:
            lusername = str(item[1])
            lemail = str(item[2])
            lpassword = str(item[3])
            lurl = str(item[4])
            let = ["[#0E7C83]" + str(number) + "[/#0E7C83]", "[#12FFA4]" + str(lusername) + "[/#12FFA4]", "[#12FFA4]" + str(
                lemail) + "[/#12FFA4]", "[#12FFA4]" + str(lpassword) + "[/#12FFA4]", "[#12FFA4]" + str(lurl) + "[/#12FFA4]"]
            number += 1
        ...
        items = c.fetchall()
        number = 1
        table_date = [["[#14BDFF]Username[/#14BDFF]", "[#14BDFF]Email[/#14BDFF]",
                       "[#14BDFF]Password[/#14BDFF]", "[#14BDFF]Url[/#14BDFF]"]]
        ...
        for item in items:
            ...
            if re.search(pattern, lusername):
                let = ["[#12FFA4]" + str(lusername) + "[/#12FFA4]", "[#12FFA4]" + str(lemail) + "[/#12FFA4]",
                       "[#12FFA4]" + str(lpassword) + "[/#12FFA4]", "[#12FFA4]" + str(lurl) + "[/#12FFA4]"]
                table_date.append(let)
            else:
                break
        print(tabulate(table_date, tablefmt="plain"))
...
def safe_note_search(pattern):
    ...
    if pattern == "all":
        ...
        for note in notes:
            ...
            print("[#0E7C83]"+ltime + "[/#0E7C83]" + "   " +
                  "[#12FFA4]" + lnotes + "[/#12FFA4]")
...
def delete_item():
    ...
    for item in items:
        ...
        lpassword = str(item[3])
        lurl = str(item[4])
        let = ["[#0E7C83]" + str(lid) + "[/#0E7C83]", "[#12FFA4]" + str(lusername) + "[/#12FFA4]", "[#12FFA4]" + str(
            lemail) + "[/#12FFA4]", "[#12FFA4]" + str(lpassword) + "[/#12FFA4]", "[#12FFA4]" + str(lurl) + "[/#12FFA4]"]
        number += 1
        table_date.append(let)
    print(tabulate(table_date, tablefmt="plain"))
...
def delete_note():
    ...
    for note in notes:
        ...
        print("[#0E7C83]" + lid + ")[/#0E7C83] "+"[#0E7C83]"+ltime +
              "[/#0E7C83]" + "   " + "[#12FFA4]" + lnotes + "[/#12FFA4]")
...
def repit(password):
    ...
    def r2(a, passwd):
        try:
            ...
            Dencrypt(passwd)
            print(
                "[#14BDFF]What is the name of the item[/#14BDFF] (type all to see all)[#14BDFF];[/#14BDFF]")
            a(str(input(": ")))
            Encrypt()
    ...
    print(str(up) + "\n" + str(med) + "[underline][#14BDFF]Menu[/#14BDFF][/underline]" + str(
